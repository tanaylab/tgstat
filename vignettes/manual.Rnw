...
having tgstat rely on the reference BLAS will result in very poor performance and is strongly discouraged. If your R implementation uses an optimized BLAS, set \texttt{options(tgs\_use.blas=TRUE)} to allow tgstat to make BLAS calls. Otherwise, set \texttt{options(tgs\_use.blas=FALSE)} (default) which instructs tgstat to avoid BLAS and instead rely only on its own optimization methods (e.g. multiprocessing). If in doubt, it is possible to run one of tgstat's CPU intensive functions (e.g. \texttt{tgs\_cor}) and compare its run time under both \texttt{options(tgs\_use.blas=FALSE)}.
