        ...
        if (!TGStat::launch_process()) {     // child process
            ...
            for (int ipoint1 = 0; ipoint1 < num_points; ++ipoint1) {
                for (int ipoint2 = spoint; ipoint2 < epoint; ++ipoint2) {
                    ...
                    bool do_dist_calc = isnan(d(ipoint1, ipoint2));
...
//                        SemLocker sl(g_tgstat->shm_sem());
                        ...
                        if (nns[ipoint1] != nns[ipoint2]) {
                            ...
                            if (!isnan(dist2 = d(ipoint2, nn1)) && fabs(dist1 - dist2) > knn_d_thr ||
                                !isnan(dist1 = d(ipoint1, nn2)) && fabs(dist1 - d(ipoint2, nn2)) > knn_d_thr ||
...
//if (!isnan(dist2 = d(ipoint2, nn1)) && fabs(dist1 - dist2) > knn_d_thr)
//printf("\tReason: 1\n");
//else if (!isnan(dist1 = d(ipoint1, nn2)) && fabs(dist1 - d(ipoint2, nn2)) > knn_d_thr)
//printf("\tReason: 2\n");
...
if (isnan(res[i]))
++num_nans;
printf("Saved %d distance calculations\n", num_nans - num_points);
...
SEXP tgs_dist(SEXP _x, SEXP _attrs, SEXP _tidy, SEXP _threshold, SEXP _rrownames, SEXP _envir)
{
	...
	try {
        ...
        int num_points = nrows(_x);
        int num_dims = ncols(_x);
        ...
        int num_cores = max(1, (int)sysconf(_SC_NPROCESSORS_ONLN));
        int num_processes = min(num_points / 2, num_cores);
        double num_row4process = num_points / (double)num_processes;
        ...
        for (int iprocess = 0; iprocess < num_processes; ++iprocess) {
            if (!TGStat::launch_process()) {     // child process
                int srow[2] = { (int)(iprocess * num_row4process / 2.), (int)(num_points - (iprocess + 1) * num_row4process / 2.) };
                int erow[2] = { (int)((iprocess + 1) * num_row4process / 2.), (int)(num_points - iprocess * num_row4process / 2.) };
                size_t itr_idx = 0;

                for (int ipart = 0; ipart < 2; ipart++) {
                    for (int irow1 = 0; irow1 < num_points; ++irow1) {
...
SEXP tgs_dist_blas(SEXP _x, SEXP _attrs, SEXP _tidy, SEXP _threshold, SEXP _rrownames, SEXP _envir)
{
	...
	try {
        ...
        // res <- -2 * (m * t(m))
        {
            ...
            double alpha = -2;
            double beta = 0;
            F77_NAME(dsyrk)(&uplo, &trans, &num_points, &num_dims, &alpha, mem.m, &num_points, &beta, mem.res, &num_points);
            check_interrupt();
            progress.report(1);
        }
        ...
        if (nan_in_vals) {
            ...
            // res <- num_dims * m^2 %*% t(mask) + num_dims * mask %*% t(m^2) + num_dims * res
            {
                ...
                F77_NAME(dsyr2k)(&uplo, &trans, &num_points, &num_dims, &alpha, mem.m, &num_points, mem.mask, &num_points, &beta, mem.res, &num_points);
                check_interrupt();
                progress.report(1);
            }
            ...
            // n <- mask %*% t(mask)
            {
                ...
                F77_NAME(dsyrk)(&uplo, &trans, &num_points, &num_dims, &alpha, mem.mask, &num_points, &beta, mem.n, &num_points);
                check_interrupt();
                progress.report(1);
            }
...
//}
        ...
        if (tidy) {
            ...
            for (int ipoint1 = 0, idx = 0; ipoint1 < num_points; ++ipoint1) {
                idx += ipoint1 + 1;
                for (int ipoint2 = ipoint1 + 1; ipoint2 < num_points; ++ipoint2) {
